...
from profitability_report import report_bytes
...
PASSWORD = "cheap"
REPORT_PASSWORD = "showmethemoney"   # unlocks the in-app profitability report
...
def load_config():
    ...
    c["branded_labor_adder_cost"]    = _scost("branded_labor_adder", c["branded_labor_adder"])
    c["two_disp_labor_adder_cost"]   = _scost("two_disp_labor_adder", c["two_disp_labor_adder"])
    c["shipping_handling_cost"]      = _scost("shipping_handling", c["shipping_handling"])
    raw_lcost = _scost("labor_daily_rate")
    c["labor_daily_rate_cost"]       = raw_lcost if raw_lcost is not None else c["labor_daily_rate"]
    ...
    c["gas_default_dispenser_spacing_ft"]    = _sval("gas_default_dispenser_spacing_ft", 28)
    ...
    c["diesel_default_dispenser_spacing_ft"] = _sval("diesel_default_dispenser_spacing_ft", 24)
    ...
    c["light_unit_price_standard_cost"] = _mcost("light_unit_price_standard", c["light_unit_price_standard"])
    c["light_unit_price_lsi_cost"]      = _mcost("light_unit_price_lsi", c["light_unit_price_lsi"])
    ...
    c["distance_adder_per_install_day_cost"] = _mcost("distance_adder_per_install_day", 600)
...
def auto_dimensions(canopy_type, dispensers, left_overhang, right_overhang,
                    spacing, fuel_type, cfg):
    n_per_row = dispensers // 2 if canopy_type == "Stacked" else dispensers
    width = left_overhang + (n_per_row - 1) * spacing + right_overhang
    base_depth_key = ("gas_default_depth_ft" if fuel_type == "Gas"
                      else "diesel_default_depth_ft")
...
def compute_quote(*, dispensers, canopy_type, double_col, fuel_type,
                  branded, brand_name, customer_supplied_imaging,
                  include_mid_material, mid_brand, include_mid_labor, mid_labor_amt,
                  left_overhang, right_overhang, spacing,
                  width, depth, distance_miles,
                  include_demo, demo_retail, demo_cost,
                  tax_rate, gp_rate, light_type, light_count,
                  flood_light_count, flood_light_unit_retail,
                  company_key, cfg):
    ...
    shipping = (cfg["shipping_handling"]
                if (branded and not customer_supplied_imaging) else 0)
    ...
    tax = _round2(retail_material * tax_rate)
    ...
    true_cost_material = (acm_cost + steel_primary_cost + decking_cost + lights_cost
                          + flood_lights_cost + misc_cost + steel_secondary_cost
                          + shipping_cost + brand_imaging_cost + mid_material_cost)
    true_cost_labor    = (base_labor_cost + branded_labor_add_cost
                          + two_disp_labor_add_cost + mid_labor_cost
                          + travel_adder_cost + demo_cost_eff)
    ...
    return {
        ...
        "profit":             profit,
        # ---- per-line COSTS (additive v14: profitability report) ----
        "gp_rate":              gp_rate,
        "acm_cost":             acm_cost,
        "steel_cost":           steel_primary_cost,
        "decking_cost":         decking_cost,
        "misc_cost":            misc_cost,
        "lights_cost":          lights_cost,
        "flood_lights_cost":    flood_lights_cost,
        "steel_secondary_cost": steel_secondary_cost,
        "shipping_cost":        shipping_cost,
        "brand_imaging_cost":   brand_imaging_cost,
        "mid_material_cost":    mid_material_cost,
        "base_labor_cost":      base_labor_cost,
        "branded_labor_add_cost":  branded_labor_add_cost,
        "two_disp_labor_add_cost": two_disp_labor_add_cost,
        "mid_labor_cost":       mid_labor_cost,
    }


def compute_imaging_only(*, brand_name, dispensers, customer_supplied_imaging,
                         labor_days, labor_misc_amt, distance_miles,
                         include_mid_material, mid_brand, include_mid_labor,
                         mid_labor_amt, tax_rate, gp_rate, company_key, cfg):
    ...
    if distance_miles is not None and distance_miles > threshold:
        ...
        travel_adder_cost   = labor_days * cfg.get("distance_adder_per_install_day_cost", 0)
    else:
        travel_adder_retail = 0
        travel_adder_cost   = 0
    ...
    tax = _round2(material_total * tax_rate)
    final = material_total + installation_total + tax

    # Profitability (best-effort)
    base_labor_cost = labor_days * cfg["labor_daily_rate_cost"]
    shipping_cost   = cfg["shipping_handling_cost"] if (not customer_supplied_imaging) else 0
    brand_imaging_cost = imaging_amt   # pure pass-through
    mid_material_cost  = (cfg["mid_prices_cost"].get(mid_brand, mid_material)
                          if include_mid_material and mid_brand else 0)
    true_cost = (brand_imaging_cost + shipping_cost + base_labor_cost
                 + labor_misc_amt + mid_material_cost + mid_labor_total + travel_adder_cost)
    ...
    return {
        ...
        # ---- per-line COSTS (additive v14: profitability report) ----
        "gp_rate":            gp_rate,
        "imaging_amt_cost":   brand_imaging_cost,
        "shipping_cost":      shipping_cost,
        "mid_material_cost":  mid_material_cost,
        "mid_labor_cost":     mid_labor_total,
        "base_labor_cost":    base_labor_cost,
        "labor_misc_cost":    labor_misc_amt,
        "travel_adder_cost":  travel_adder_cost,
    }
...
def _build_proposal_data(q):
    ...
    if mode == "imaging_only":
        ...
        base.update({
            ...
            "shared": {
                "mid": {"include_material": q["include_mid_material"],
                        "include_labor":    q["include_mid_labor"],
                        "brand":            q["mid_brand"],
                        "material_amt":     r["mid_material"],
                        "labor_amt":        r["mid_labor"]},
    ...
    base.update({
        ...
        "mid": {"include_material": q["include_mid_material"],
                "include_labor":    q["include_mid_labor"],
                "brand":            q["mid_brand"],
                # v14 FIX: single-canopy was dropping the price-sign amounts from
                # the total (proposal showed "Price Sign Total: $0.00"). Pass them
                # through exactly like the double-canopy branch already does.
                "material_amt":     q["mid_material_amt"],
                "labor_amt":        q["mid_labor_amt"]},
        "demo": {"include": q["include_demo"],
                 "retail":  q["demo_retail"],
                 "cost":    q["demo_cost"]},
        "pricing": dict(cd["pricing"], tax_rate=q["tax_rate"],
                         mid_material=r["mid_material"], mid_labor=r["mid_labor"]),
    })
    return base
...
def _next_quote_number():
    # The quote tracker that used to supply the running daily sequence was
    # removed on 2026-06-01. Quote numbers keep the Q-YYYYMMDD-NNN format,
    # but with no shared counter the sequence resets to -001 each day.
    today = dt.date.today().strftime("%Y%m%d")
    return f"Q-{today}-001"
...
distance_miles = st.number_input(
    ...
    help=(f"If over {threshold} miles, a travel adder is silently added to "
...
quote_type = st.radio("What is this quote for?",
                       ["Canopies", "Imaging Only"], horizontal=True, index=0,
                       help=("Imaging Only = re-imaging an existing canopy "
...
def _password_gate(label="Override GP %", *, default_gp, key_prefix=""):
    ...
    if st.checkbox(label, value=False, key=f"{key_prefix}_ov_gp"):
        ...
        if pwd == PASSWORD:
            gp_pct = st.number_input("Custom GP (%)", 0.0, 200.0,
                                      default_gp * 100, 1.0,
...
def render_canopy_inputs(*, key_prefix, fuel_locked=None, gp_default=None,
    ...
    height_choice = st.radio("Canopy height",
                              ["16.5 ft (standard)", "20 ft"], horizontal=True, index=0,
    ...
    if fuel_locked is None:
        fuel_type = st.radio("Fuel type", ["Gas", "Diesel"], horizontal=True, index=0,
    ...
    canopy_type = st.radio("Canopy type",
                            ["Dive-in", "Stacked", "In-line"], horizontal=True, index=0,
    ...
    if canopy_type == "Dive-in":
        col_arr = st.radio("Column arrangement",
                            ["Single column", "Double column"], horizontal=True,
                            index=0, key=f"{key_prefix}_colarr")
        double_col = (col_arr == "Double column")
    columns = column_count(canopy_type, dispensers, double_col)
    st.markdown(f'<div class="auto-readout">Columns: <b>{columns}</b> '
                f'(auto-calculated)</div>', unsafe_allow_html=True)
    ...
    with oc1:
        ov_lo = st.checkbox(f"Override LO (default {default_lo:g}′)",
                             value=False, key=f"{key_prefix}_ov_lo")
        left_overhang = (st.number_input("Left Overhang (ft)", 0.0, 50.0,
                                          default_lo, 1.0,
                                          key=f"{key_prefix}_lo")
                         if ov_lo else default_lo)
    with oc2:
        ov_sp = st.checkbox(f"Override spacing (default {default_spacing:g}′)",
                             value=False, key=f"{key_prefix}_ov_sp")
        spacing = (st.number_input("Dispenser spacing (ft)", 1.0, 100.0,
                                     default_spacing, 1.0,
                                     key=f"{key_prefix}_sp")
                   if ov_sp else default_spacing)
    with oc3:
        ov_ro = st.checkbox(f"Override RO (default {default_ro:g}′)",
                             value=False, key=f"{key_prefix}_ov_ro")
        right_overhang = (st.number_input("Right Overhang (ft)", 0.0, 50.0,
                                            default_ro, 1.0,
                                            key=f"{key_prefix}_ro")
                          if ov_ro else default_ro)
    out["left_overhang"] = left_overhang
    out["right_overhang"] = right_overhang
    out["spacing"] = spacing

    auto_w, auto_d = auto_dimensions(canopy_type, dispensers, left_overhang,
                                      right_overhang, spacing, fuel_type, cfg)
    ov_w = st.checkbox(f"Override width (auto: {auto_w:g}′)", value=False,
                        key=f"{key_prefix}_ov_w")
    width = (st.number_input("Width (ft)", 1.0, 500.0, float(auto_w), 1.0,
                              key=f"{key_prefix}_w")
             if ov_w else float(auto_w))
    ov_d = st.checkbox(f"Override depth (auto: {auto_d:g}′)", value=False,
                        key=f"{key_prefix}_ov_d")
    depth = (st.number_input("Depth (ft)", 1.0, 500.0, float(auto_d), 1.0,
                              key=f"{key_prefix}_d")
             if ov_d else float(auto_d))
    out["width"] = width
    out["depth"] = depth
    ...
    # ── Canopy Lights ──
    light_type = st.radio("Light type",
                           ["Standard", "LSI", "Customer-provided"],
                           horizontal=True, index=0, key=f"{key_prefix}_lt",
                           help=(f"Standard ${int(cfg['light_unit_price_standard'])}/light; "
    ...
    ov_lights = st.checkbox(f"Override number of canopy lights (default {default_lights})",
                             value=False, key=f"{key_prefix}_ov_lights")
    if ov_lights:
        light_count = int(st.number_input("Number of canopy lights",
                                           min_value=0, max_value=400,
                                           value=default_lights, step=1,
    ...
    if needs_reconfirm:
        light_reconfirmed = st.checkbox(
            ...
            value=False, key=f"{key_prefix}_light_reconfirm")
    ...
    flood_light_count = int(st.number_input(
        "Number of flood lights", min_value=0, max_value=400, value=0, step=1,
    ...
    if flood_light_count > 0:
        if st.checkbox(f"Override flood light rate (default ${flood_default_rate}/light)",
                       value=False, key=f"{key_prefix}_ov_flood_rate"):
            flood_light_unit_retail = int(st.number_input(
                "Flood light rate ($/light)", min_value=flood_default_rate,
                value=flood_default_rate, step=25, key=f"{key_prefix}_flood_rate",
    ...
    branded_choice = st.radio("Is this a branded site?",
                               ["No (Unbranded)", "Yes (Branded)"],
                               horizontal=True, index=0, key=f"{key_prefix}_branded")
    branded = (branded_choice == "Yes (Branded)")
    brand_name = None
    customer_supplied_imaging = False
    if branded:
        brand_options = list(cfg["brand_imaging"].keys())
        brand_name = st.selectbox("Brand", brand_options, index=0,
                                   key=f"{key_prefix}_brand")
        supply_choice = st.radio(
            "Imaging material supplied by:",
            [f"{company_key} provides", "Customer provides"],
            horizontal=True, index=0, key=f"{key_prefix}_supply",
            help=("Pick who supplies imaging material. Auto-flips to Customer "
        ...
        if not customer_supplied_imaging and not has_imaging_price:
            ...
            st.warning(f"No imaging price on file for **{brand_name}** — "
    ...
    eff_default = (gp_default - LARGE_CANOPY_GP_REDUCTION) if large_canopy else gp_default
    gp_rate, gp_overridden = _password_gate(gp_label,
                                             default_gp=eff_default,
                                             key_prefix=key_prefix)
    if large_canopy and not gp_overridden:
        st.caption(f"📉 {dispensers} dispensers (≥ 6) — GP reduced 3 points "
...
if quote_type == "Imaging Only":
    st.markdown('<div class="section-title">Imaging Configuration</div>',
                unsafe_allow_html=True)
    brand_options = list(cfg["brand_imaging"].keys())
    brand_name = st.selectbox("Brand", brand_options, index=0, key="img_brand")
    dispensers = int(st.number_input("Number of dispensers (drives imaging price)",
                                       min_value=2, max_value=10,
                                       value=int(cfg["default_dispensers"]), step=1,
                                       key="img_disp"))
    supply_choice = st.radio(
        "Imaging material supplied by:",
        [f"{company_key} provides", "Customer provides"],
        horizontal=True, index=0, key="img_supply",
    ...
    st.markdown('<div class="section-title">Installation Labor</div>',
                unsafe_allow_html=True)
    labor_days = int(st.number_input("Labor days (uses canopy labor formula)",
                                       min_value=1, max_value=60, value=5, step=1,
                                       key="img_days"))
    labor_misc_amt = int(st.number_input(
        "Additional labor / misc expenses ($)",
        min_value=0, value=0, step=100, key="img_misc",
        help="User-input dollar amount. Added to installation labor (non-taxable)."))
    st.caption(f"Daily labor rate: ${int(cfg['labor_daily_rate']):,}/day "
               f"(× labor days × (1 + GP) + misc + travel = installation labor; "
               "non-taxable).")

    # MID 4-way (with USER-INPUT labor amount)
    st.markdown('<div class="section-title">MID / Price Sign</div>',
                unsafe_allow_html=True)
    include_mid_material = st.radio("Include price sign **material**?",
    ...
    if include_mid_material:
        ...
        mid_brand = st.selectbox("Price sign brand", mid_options, index=di,
                                   key="img_mid_brand")
    include_mid_labor = st.radio("Include price sign **installation labor**?",
                                  ["No", "Yes"], horizontal=True, index=0,
                                  key="img_mid_lab") == "Yes"
    mid_labor_amt = 0
    if include_mid_labor:
        # v13: imaging-only mode uses USER INPUT amount (no MID_Labor lookup).
        st.caption("Imaging-only mode — enter the price sign installation amount manually.")
        mid_labor_amt = int(st.number_input(
            "Price sign installation amount ($)",
            min_value=0, value=8000, step=500, key="img_mid_labor_amt"))
    if include_mid_labor and not include_mid_material:
        ...
        try:
            ...
            di = (mid_options.index("Unbranded") if "Unbranded" in mid_options else 0)
        mid_brand = st.selectbox("Price sign brand (for installation labeling)",
                                   mid_options, index=di, key="img_mid_brand_lab")

    st.markdown('<div class="section-title">GP Override (optional)</div>',
                unsafe_allow_html=True)
    gp_rate, gp_overridden = _password_gate("Override GP %",
                                             default_gp=predefined_gp,
                                             key_prefix="img")

    st.markdown('<br>', unsafe_allow_html=True)
    calc = st.button("⚡ Calculate Imaging Price", type="primary",
                      use_container_width=True, key="img_calc_btn")
    if calc:
        result = compute_imaging_only(
            brand_name=brand_name, dispensers=dispensers,
            customer_supplied_imaging=customer_supplied_imaging,
            labor_days=labor_days, labor_misc_amt=labor_misc_amt,
            distance_miles=distance_miles,
            include_mid_material=include_mid_material, mid_brand=mid_brand,
            include_mid_labor=include_mid_labor, mid_labor_amt=mid_labor_amt,
            tax_rate=tax_rate, gp_rate=gp_rate,
            company_key=company_key, cfg=cfg,
        )
...
# ════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown('<div class="section-title">Canopy Configuration</div>',
                unsafe_allow_html=True)
    canopy_count_choice = st.radio(
        "Single or Double canopy quote?",
        ["Single Canopy", "Double Canopy (Gas + Diesel)"],
        horizontal=True, index=0, key="canopy_count_choice",
        help=("Single = one canopy on this quote. Double = two canopies "
    ...
    if not is_double:
        ...
        canopy_inputs = render_canopy_inputs(
            key_prefix="single", fuel_locked=None,
            gp_default=predefined_gp, gp_label="Override GP %")
        canopy_inputs_list = [canopy_inputs]
    else:
        # ── DOUBLE CANOPY MODE ──
        st.markdown('<div class="canopy-title">⛽ GAS CANOPY</div>',
                    unsafe_allow_html=True)
        gas_inputs = render_canopy_inputs(
            key_prefix="gas", fuel_locked="Gas",
            gp_default=predefined_gp, gp_label="Override GP % (Gas canopy)")
        st.markdown('<div class="canopy-title">⛽ DIESEL CANOPY</div>',
                    unsafe_allow_html=True)
        ...
        st.caption(f"💡 Default Diesel canopy GP = Gas canopy GP − 3% "
        ...
        diesel_inputs = render_canopy_inputs(
            key_prefix="diesel", fuel_locked="Diesel",
            gp_default=canopy2_default, gp_label="Override GP % (Diesel canopy)")
        canopy_inputs_list = [gas_inputs, diesel_inputs]

    # ── Shared MID / Price Sign ──
    st.markdown('<div class="section-title">MID / Price Sign (shared)</div>',
                unsafe_allow_html=True)
    include_mid_material = st.radio("Include price sign **material**?",
                                     ["No", "Yes"], horizontal=True, index=0,
                                     key="shared_mid_mat") == "Yes"
    mid_brand = None
    mid_brand_options = list(cfg["mid_prices"].keys())
    if include_mid_material:
        # Suggest the first canopy's brand if it's branded
        suggested = (canopy_inputs_list[0].get("brand_name")
                      if canopy_inputs_list[0].get("branded") else None)
        try:
            di = mid_brand_options.index(suggested) if suggested else (
                 mid_brand_options.index("Unbranded") if "Unbranded" in mid_brand_options else 0)
        except ValueError:
            di = (mid_brand_options.index("Unbranded")
                   if "Unbranded" in mid_brand_options else 0)
        mid_brand = st.selectbox("Price sign brand", mid_brand_options, index=di,
                                   key="shared_mid_brand")
    include_mid_labor = st.radio("Include price sign **installation labor**?",
                                  ["No", "Yes"], horizontal=True, index=0,
                                  key="shared_mid_lab") == "Yes"
    mid_labor_amt = 0
    if include_mid_labor:
        default_mid_labor = int(cfg["mid_labor_by_company"].get(company_key,
                                                                  cfg["mid_labor_by_company"].get("APEC", 11000)))
        st.caption(f"Default {company_key} price-sign installation labor: ${default_mid_labor:,}")
        if st.checkbox("Override price sign installation amount", value=False,
                        key="shared_mid_lab_ov"):
            mid_labor_amt = int(st.number_input(
                "Custom price sign installation ($)", min_value=0,
                value=default_mid_labor, step=500, key="shared_mid_lab_amt"))
        else:
    ...
    if include_mid_labor and not include_mid_material:
        ...
        try:
            ...
            di = (mid_brand_options.index("Unbranded")
                   if "Unbranded" in mid_brand_options else 0)
        mid_brand = st.selectbox("Price sign brand (for installation labeling)",
                                   mid_brand_options, index=di,
                                   key="shared_mid_brand_lab")

    # ── Existing Canopy Removal (shared) ──
    st.markdown('<div class="section-title">Existing Canopy Removal (shared)</div>',
                unsafe_allow_html=True)
    include_demo = st.radio("Include existing canopy removal?",
                             ["No", "Yes"], horizontal=True, index=0,
                             key="shared_demo") == "Yes"
    demo_retail = 0
    demo_cost   = 0
    if include_demo:
        ...
        st.caption(f"Defaults — Retail: ${default_demo_retail:,} / "
                    f"Cost: ${default_demo_cost:,}")
        if st.checkbox("Override removal price", value=False, key="shared_demo_ov"):
            rc1, rc2 = st.columns(2)
            with rc1:
                demo_retail = int(st.number_input("Removal retail ($)",
                                                    min_value=0,
                                                    value=default_demo_retail,
                                                    step=500, key="shared_demo_r"))
            with rc2:
                demo_cost   = int(st.number_input("Removal cost ($)",
                                                    min_value=0,
                                                    value=default_demo_cost,
                                                    step=500, key="shared_demo_c"))
        else:
            demo_retail = default_demo_retail
            demo_cost   = default_demo_cost

    st.markdown('<br>', unsafe_allow_html=True)
    calc = st.button("⚡ Calculate Canopy Price", type="primary",
                      use_container_width=True, key="canopies_calc_btn")
    ...
    if calc:
        for ci, cinp in enumerate(canopy_inputs_list):
            if cinp["needs_light_reconfirm"] and not cinp["light_reconfirmed"]:
                ...
                st.error(f"⚠️ Please tick the box to confirm the non-standard "
    ...
    if calc:
        ...
        for cinp in canopy_inputs_list:
            r = compute_quote(
                ...
                width=cinp["width"], depth=cinp["depth"],
                distance_miles=distance_miles,
                include_demo=False,  # demo applied at quote level (shared)
                demo_retail=0, demo_cost=0,
                tax_rate=tax_rate, gp_rate=cinp["gp_rate"],
                light_type=cinp["light_type"], light_count=cinp["light_count"],
        ...
        st.session_state["last_quote"] = {
            ...
            "mid_material_amt": (cfg["mid_prices"].get(mid_brand, 0)
                                  if include_mid_material and mid_brand else 0),
            "mid_material_cost_amt": (cfg["mid_prices_cost"].get(mid_brand,
                                       cfg["mid_prices"].get(mid_brand, 0))
                                  if include_mid_material and mid_brand else 0),
            "include_demo": include_demo,
            "demo_retail":  demo_retail, "demo_cost": demo_cost,
            "tax_rate":     tax_rate, "tax_rate_pct": tax_rate_pct,
            "quote_number": None,
            "quote_date":   dt.date.today().strftime("%B %d, %Y"),
        }
...
if "last_quote" in st.session_state:
    ...
    st.markdown('<div class="section-title">Quote Result</div>',
                unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">Project Summary</div>',
                unsafe_allow_html=True)
    summary = [
        ("Quote for", q["brand_company"]),
        ("Quote mode", {"single_canopy": "Single canopy",
                         "double_canopy": "Double canopy (Gas + Diesel)",
                         "imaging_only": "Imaging only"}[mode]),
    ...
    if mode == "imaging_only":
        ...
        summary += [
            ("Brand", q["brand_name"]),
            ("Imaging supplier",
             "Customer-supplied" if q["customer_supplied_imaging"]
        ...
        for ci, (cinp, r) in enumerate(zip(q["canopies"], q["results"])):
            tag = (f"({cinp['fuel_type']})"
                    if mode == "double_canopy" else "")
            summary.append((f"Canopy {ci+1} {tag}".strip(),
                             f'{cinp["fuel_type"]} {cinp["canopy_type"]} '
                             f'/ {cinp["dispensers"]} disp / '
                             f'{r["columns"]} cols / GP {cinp["gp_rate"]*100:.0f}%'))
            summary.append((f"  └─ Branding",
                             (f'Yes — {cinp["brand_name"]}'
                              + (' (Customer-supplied)'
                                 if cinp["customer_supplied_imaging"] else ''))
                             if cinp["branded"] else "No"))
            summary.append((f"  └─ Lights",
                             f'{cinp["light_count"]} {cinp["light_type"]}'
                             + (f' + {cinp["flood_light_count"]} flood'
                                if cinp["flood_light_count"] else '')))
    if mode != "imaging_only":
        ...
        if q.get("include_demo"):
            summary.append(("Existing canopy removal (shared)",
    ...
    if items_not_inc:
        st.markdown('<div class="section-title">Items Not Included</div>',
                    unsafe_allow_html=True)
    ...
    # ── Generate Proposal ────────────────────────────────────────────────
    st.markdown('<div class="section-title">Generate Proposal</div>',
                unsafe_allow_html=True)
    ...
    # ── Profitability Report (internal Excel) — password protected ───────
    # Leadership / code-access users unlock with REPORT_PASSWORD. Regular
    # salespeople cannot see profitability in-app (they hand off the Word
    # proposal, and profitability is reconstructed separately from the docx).
    st.markdown("&nbsp;", unsafe_allow_html=True)
    st.markdown('<div class="section-title">Profitability Report (internal)</div>',
                unsafe_allow_html=True)
    _rep_pw = st.text_input("Enter password to unlock the profitability report",
                             type="password", key="report_pw")
    if _rep_pw == REPORT_PASSWORD:
        rep = st.button("📊 Create Profitability Report (internal)",
                         use_container_width=True, key="profit_report_btn")
        if rep:
            try:
                st.session_state["profit_report_bytes"] = report_bytes(q)
                _qn = q.get("quote_number") or "DRAFT"
                _street = _street_name_for_filename(q.get("cust_street", ""))
                st.session_state["profit_report_filename"] = (
                    f"Profitability_{_qn}" + (f" ({_street})" if _street else "") + ".xlsx")
                st.success("Profitability report generated (internal — shows true costs).")
            except Exception as e:
                st.error(f"Could not generate profitability report: {e}")

        if "profit_report_bytes" in st.session_state:
            st.download_button(
                "⬇️ Download Profitability Report (.xlsx)",
                data=st.session_state["profit_report_bytes"],
                file_name=st.session_state["profit_report_filename"],
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                use_container_width=True,
            )
    elif _rep_pw:
        st.error("Incorrect password — profitability report is restricted.")
