...
def _shared_block(data):
    if "shared" in data and isinstance(data["shared"], dict):
        ...
        return s
    return {
        "mid":  data.get("mid", {"include_material": False, "include_labor": False, "brand": None}),
...
def _mid_state(shared_or_data):
    if "mid" in shared_or_data and isinstance(shared_or_data["mid"], dict) and (
        "include_material" in shared_or_data["mid"] or "include_labor" in shared_or_data["mid"]
    ...
    if "include_material" in m or "include_labor" in m:
        return bool(m.get("include_material")), bool(m.get("include_labor")), m.get("brand")
...
def _canopy_totals(canopy):
    ...
    gp = p.get("gp_rate", 0)
    canopy_mat_marked = (p.get("acm", 0) + p.get("steel", 0)
                          + p.get("decking", 0) + p.get("misc", 0)) * (1 + gp)
    canopy_mat = (canopy_mat_marked + p.get("steel_secondary", 0)
                  + p.get("lights", 0) + p.get("flood_lights", 0))
    canopy_inst_marked = p.get("base_labor", 0) * (1 + gp)
    canopy_inst = (canopy_inst_marked
...
def _imaging_totals(data):
    ...
    labor_marked = img.get("labor_days", 0) * img.get("labor_daily_rate", 1600) * (1 + gp)
...
def _combined_totals(data):
    ...
    tax = _round2(material_total * shared.get("tax_rate", 0.0825))
    grand_total = material_total + installation_total + tax
...
def _derive_canopy_inclusions(canopy, *, label_prefix=None):
    ...
    if canopy.get("branded"):
        if not customer_supplied:
            ...
            inc.append(f"Brand imaging installation labor only "
...
def _derive_shared_inclusions(data):
    ...
    if inc_mat and inc_lab:
        ...
        inc.append(f"Price sign installation only (customer-supplied material"
...
def _derive_general_exclusions(data):
    ...
    if (inc_mat or inc_lab) and mid_brand and mid_brand != "Unbranded" and has_unbranded:
        exc.append("Canopy imaging (by others — scope on this proposal "
...
def _add_branding_section(doc, canopy, totals, *, label_prefix=None):
    ...
    if branded:
        if customer_supplied:
            band_label = f"{base}  —  {canopy.get('brand_name','')}  (Material by Customer)"
        else:
    ...
    if branded and not customer_supplied:
        _add_para(cell, IMAGING_DISCLAIMER, size=9, bold=True, italic=True,
                  color=PALETTE["accent"], space_after=4)
    ...
    if branded and not customer_supplied:
        section_name = ("Branding" if not label_prefix
                        else label_prefix.title() + " Branding")
        _add_section_subtotal(doc, totals["branding_mat"], totals["branding_inst"],
                              section_name=section_name)
    else:
        if customer_supplied:
            _add_section_subtotal_note(doc,
                "Material provided by customer — not included in this quote")
        else:
...
def _add_price_sign_section(doc, data, totals):
    ...
    brand = brand or ""
    mat_amt  = totals.get("price_sign_mat", totals.get("mid_material", 0))
    inst_amt = totals.get("price_sign_inst", totals.get("mid_labor", 0))
    ...
    if has_unbranded and brand and brand != "Unbranded" and (inc_mat or inc_lab):
        _add_para(cell,
                  ...
                  size=9, italic=True, color=PALETTE["accent"], space_after=2)
...
def _add_demolition_section(doc, data, totals):
    ...
    _set_cell_margins(cell, top=100, left=160, bottom=80, right=160)
    _clear_default_para(cell)
    _add_para(cell, "Scope of work:", size=9, bold=True,
              color=PALETTE["text_emphasis"], space_after=2)
    canopies = _canopies_list(data)
    ...
    _add_para(cell, "", size=2, space_after=2)
    _add_single_subtotal(doc, totals.get("demo_retail", 0),
                         label="Existing Canopy Removal")
...
def _add_subhead(cell, text, color):
    _add_para(cell, text, size=9, bold=True, color=color, space_after=2,
              space_before=4)
...
def _add_incl_excl(doc, data):
    ...
    for c in canopies:
        ...
        for it in items:
            _add_para(inc_cell, f"•  {it}", size=9, color=PALETTE["text_emphasis"], space_after=1)
    ...
    for c in canopies:
        ...
        if per_exc:
            ...
            for it in per_exc:
                _add_para(exc_cell, f"•  {it}", size=9, color=PALETTE["text_emphasis"], space_after=1)
...
def _spec_rows_for_canopy(canopy):
    col_arr = ("Double column" if (canopy["type"] == "Dive-in" and canopy.get("double_col"))
               else "Single column")
    canopy_label = (f"{canopy['type']} ({col_arr})" if canopy["type"] == "Dive-in"
                    else canopy["type"])
    branded_label = (f"Yes — {canopy.get('brand_name','')}" if canopy.get("branded")
                     else "No")
...
def _spec_rows(data):
    ...
    if inc_mat and inc_lab:
        ...
        price_sign_label = f"Installation only — {mid_brand or ''}" if mid_brand else "Installation only"
    else:
        price_sign_label = "No"
    inserted = []
    for k, v in rows:
...
def _build_customer_specs(doc, data):
    ...
    _add_para(left, "TERMS", size=8, bold=True,
              color=PALETTE["text_emphasis"], space_after=2)
    _add_para(left, "Quote valid for 30 days", size=10, space_after=1)
    _add_para(left, "Payment: 50% deposit / 50% at start of work",
    ...
    if mode == "imaging_only":
        img = data.get("imaging", {})
        _add_para(right, f"Imaging Only  •  {img.get('brand','—')}",
                  size=10, bold=True, color=PALETTE["accent"], space_after=4)
        rows = [
            ("Quote Type", "Imaging Only"),
            ("Brand", img.get("brand", "—")),
            ("Dispensers", str(img.get("dispensers", "—"))),
            ("Imaging Supplier",
             "Customer" if img.get("customer_supplied_imaging") else f"{data.get('company_key','APEC')} provides"),
        ...
        _add_para(right, "  |  ".join(_spec_block_caption(c) for c in canopies),
                  size=10, bold=True, color=PALETTE["accent"], space_after=4)
        ...
        for ci, lab in enumerate(labels):
            ...
            _add_para(hcell, lab, size=8, bold=True,
                      color=PALETTE["text_emphasis"], space_after=0)
        ...
        for ri in range(n):
            for side, rows in enumerate((rows_a, rows_b)):
                ...
                _add_para(vcell, v, size=8, bold=True,
                          color=PALETTE["text_emphasis"], space_after=0,
                          align=WD_ALIGN_PARAGRAPH.RIGHT)
        shared = _shared_block(data)
        inc_mat, inc_lab, mid_brand = _mid_state(shared)
        if inc_mat and inc_lab:
            ...
            ps = f"Installation only — {mid_brand or ''}" if mid_brand else "Installation only"
        else:
            ps = "No"
        demo_lbl = "Yes" if shared.get("demo", {}).get("include") else "No"
        _add_para(right, "", size=2, space_after=0)
        _add_para(right, "SHARED", size=8, bold=True,
                  color=PALETTE["text_emphasis"], space_after=2)
        _render_spec_table(right, [("Price Sign", ps), ("Existing Canopy Removal", demo_lbl)],
                           [(USABLE_W - 3.3 - 0.4) / 2]*2)
    else:
        _add_para(right, _spec_block_caption(data), size=10, bold=True,
                  color=PALETTE["accent"], space_after=4)
...
def sample_data():
    return {
        ...
        "canopy": {
            ...
            "width": 94, "depth": 24,
            "left_overhang": 10, "right_overhang": 10, "spacing": 28,
            "labor_days": 7, "light_type": "Standard", "light_count": 16,
            "flood_light_count": 0,
        },
        ...
        "pricing": {
            ...
            "lights": 2_400, "flood_lights": 0, "misc": 2_500,
...
if __name__ == "__main__":
    out = build_proposal(sample_data(),
                         Path(__file__).parent / "Proposal_v13_Sample.docx")
    print(f"Wrote {out}")
